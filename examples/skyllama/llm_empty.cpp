...
int main_llm_loop(int argc, char ** argv) {

    // Redirect standard error to the file
    FILE* errFile = freopen("G:\\output_err.txt", "w", stderr);
    if (!errFile) {
        std::cerr << "Failed to redirect stderr to output_err.txt\n";
        return 1;
    }
    // Redirect standard output to the file
    FILE* outFile = freopen("G:\\output.txt", "w", stdout);
    if (!outFile) {
        std::cerr << "Failed to redirect stdout to output.txt\n";
        return 1;
    }
    ...
    g_params = &params;
    std::cerr << "common_params_parse\n";
    if (!common_params_parse(argc, argv, params, LLAMA_EXAMPLE_MAIN, fake_print_usage)) {
        std::cerr << "Problems parsing args\n";
    }
    std::cerr << "suceess common_params_parse\n";
    // Flush the streams to ensure all output is written
    std::cout.flush();
    std::cerr.flush();
    atexit([]() {
        ...
        if (!outFile) {
            std::cerr << "Failed to restore stdout\n";
        }

        FILE* errFile = freopen("CONOUT$", "w", stderr);
        if (!errFile) {
            std::cerr << "Failed to restore stderr\n";
        }
    });
    // Optionally, close the file streams if you are done writing
    //fclose(outFile);
    //fclose(errFile);
    //common_init();
    //
...
//int broken_main_llm_loop(int argc, char** argv) {
//    while (true) {
//    }
//    return 0;
//
